...
const BattleArena = ({ fighter1, fighter2, onBackToSelect }: BattleArenaProps) => {
  ...
  const startBattle = () => {
    ...
    let fighter1HasRevived = false;
    let fighter2HasRevived = false;

    const tryResurrect = (dead: Fighter, setHealth: (v: number | ((p: number) => number)) => void) => {
      // Only applies to the "costume" fighter id
      if (dead.id !== 'custom') return false;

      // Only allow one resurrection per fighter per battle
      const alreadyRevived = dead.id === fighter1.id ? fighter1HasRevived : fighter2HasRevived;
      if (alreadyRevived) return false;

      const roll = Math.random();
      if (roll < 0.9) {
        // mark as revived so it won't happen again
        if (dead.id === fighter1.id) fighter1HasRevived = true;
        if (dead.id === fighter2.id) fighter2HasRevived = true;

        // revive with 50 HP after a short delay (allow death animations to play)
        setTimeout(() => setHealth(50), 700);
        return true;
      }
